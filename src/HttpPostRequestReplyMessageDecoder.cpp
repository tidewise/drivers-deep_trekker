...
HttpPostRequestReplyMessageDecoder::HttpPostRequestReplyMessageDecoder()
    : mReader(mRBuilder.newCharReader())
{
}

bool HttpPostRequestReplyMessageDecoder::parseJSONMessage(char const* data,
    string& errors)
{
    return mReader->parse(data, data + strlen(data), &mJData, &errors);
}

void HttpPostRequestReplyMessageDecoder::validateFieldPresent(Json::Value const& value,
